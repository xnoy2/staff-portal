...
class StaffController extends Controller
{
    ...
    public function store(StoreUserRequest $request): RedirectResponse
    {
        ...
        $user = User::create([
            ...
            'timezone'                => $request->filled('timezone') ? $request->input('timezone') : 'Europe/London',
    ...
    public function show(User $staff): Response
    {
        ...
        $recentEntries = TimeEntry::forUser($staff->id)
            ...
            ->map(fn ($e) => [
                'id'        => $e->id,
                'date'      => $e->clock_in?->copy()->setTimezone($staff->timezone)->toDateString(),
                'clock_in'  => $e->clock_in?->copy()->setTimezone($staff->timezone)->format('H:i'),
                'clock_out' => $e->clock_out?->copy()->setTimezone($staff->timezone)->format('H:i'),
                'hours'     => $e->total_hours,
                'status'    => $e->status,
                'source'    => $e->source,
            ]);
        ...
        return Inertia::render('Staff/Show', [
            ...
            'dailyLogs'        => \App\Models\DailyLog::where('user_id', $staff->id)
                ...
                ->limit(20)
                ->get(['id', 'log_date', 'status', 'photos', 'jobs', 'acknowledged_at'])
                ->map(fn ($l) => [
                    'id'           => $l->id,
